...
module Curburger
  ...
  class Client
    ...
    #   rewrite_status   - rewrite response header status to '200 OK' ?
    #   exception_status - Array of response statuses which throw exception and
    #                      stop retry attemtps
    def initialize o={}
      ...
      @read_cookies = o[:read_cookies].nil? ? false : o[:read_cookies]  ? true : false
      @use_cookies  = o[:use_cookies].nil?  ? false : o[:use_cookies]   ? true : false
      ...
      @rewrite_status   = o[:rewrite_status] ? o[:rewrite_status] : true
      @exception_status = o[:exception_status] ? o[:exception_status] : []
      @req_retry_wait =
    ...
    def cookies
      self.class.parse_cookies(self.class.parse_headers(@curb.header_str))
    end
    ...
    def initialize_curl
      ...
      @curb.enable_cookies = true if
        @curb_opts[:cookies] || @curb_opts[:read_cookies] ||
          @curb_opts[:use_cookies]
      @curb.resolve_mode = @curb_opts[:resolve_mode] || :ipv4
    end

    extend  Curburger::Headers
    extend  Curburger::Cookies
